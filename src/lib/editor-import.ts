...
async function importHtml(file: File): Promise<ImportResult> {
  ...
  const blocks: Block[] = [
    ...
    },
    {
      ...
      y: 205,
      w: 960,
      text: bodyText || "HTML 文件没有可导入的正文文本。",
      fontSize: 22,
      lineHeight: 1.55,
    },
    {
      ...
      y: 1650,
      w: 950,
      text: "导入后可双击文字直接编辑，或用 AI Agent 继续排版。",
      fontSize: 14,
      color: POSTER_MUTED,
    },
  ];
...
async function importBinaryDocument(file: File, kind: string): Promise<ImportResult> {
  ...
  const blocks: Block[] = [
    ...
    {
      id: `import-doc-info-${now}`,
      ...textBase,
      x: 72,
      y: 230,
      w: 900,
      text: `文件大小：${size}\n导入时间：${new Date().toLocaleString()}\n\n已创建为本地导入页面。PDF / PPTX 的原始版式解析需要浏览器外部转换；你可以继续把关键内容复制进页面，或导入 HTML 版本获得可编辑文本。`,
      fontSize: 22,
      lineHeight: 1.55,
    },
    {
      id: `import-doc-bg-${now}`,
      type: "image",
      x: 72,
      y: 410,
      w: 960,
    ...
    {
      id: `import-doc-footer-${now}`,
      ...textBase,
      x: 72,
      y: 1650,
      w: 960,
      text: "建议：将 PDF/PPTX 转为 HTML 后导入，可获得更好的文本拆分与再编辑效果。",
      fontSize: 14,
      color: POSTER_MUTED,
    },
  ];
