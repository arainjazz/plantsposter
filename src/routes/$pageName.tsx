...
export const Route = createFileRoute("/$pageName")({
  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        { title: `${pageName} · 植物图鉴 AI 编辑器` },
        {
          name: "description",
          content: "Canva 风格的 A3 植物科普海报编辑器，支持拖动、多选、AI 编辑与多格式导出。",
        },
        { property: "og:title", content: `${pageName} · 植物图鉴 AI 编辑器` },
...
function Editor() {
  const defaultPlantsState = Route.useLoaderData();
  ...
  // Sync URL path param 'pageName' -> activeId
  useEffect(() => {
    if (!hydrated) return;
    if (decodedPageName) {
  ...
  function deletePage(id: string) {
    ...
    if (id === activeId) {
      ...
      void navigate({
        to: "/$pageName",
        params: { pageName: nextActive.name },
        replace: true,
      });
    }
    setSelectedIds(new Set());
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      if (file.name.toLowerCase().endsWith(".json")) {
        ...
        void navigate({
          to: "/$pageName",
          params: { pageName: result.page.name },
          replace: true,
        });
      }
    } catch (e) {
